...
def main():
    packer_name = 'ali'
    cursor = select_samples('../Android_app_database', 'select filename from APK_basic_info where shellProtection like \'{}\''.format(packer_name))
    file_name_list = []
    file_path_list = []
    for s in cursor:
        file_name_list.append(s[0])
    file_path_list = find_samples('E:\\Android样本\\APK', file_name_list)
